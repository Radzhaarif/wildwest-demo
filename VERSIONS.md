...
### 2026-07-18 - Падение предметов и награды привязаны к реальным кадрам

- Тряска совпадений и заблокированных ходов, а также падение/добор предметов переведены с CSS-анимации, завершавшейся параллельным таймером, на общий покадровый `requestAnimationFrame` flow. Падение ускоряется вниз и удерживает финальную позицию реальный кадр до продолжения каскада.
- Общее окно награды переведено с независимых CSS keyframes на единую покадровую шкалу: вход окна, фон, рост иконок и появление карточек работают одинаково для наград карты/сундуков, победы в бою и повышения уровня.
- На coarse-pointer телефонах reward-фон сразу затемняется без дорогого живого blur; масштабирование фона и все анимации наград сохраняются.
- Browser-smoke теперь требует увидеть изменение transform падения между двумя отрисовками и промежуточный размер иконки как в обычном reward-окне, так и при повышении уровня.
- `version.json` поднят до `2026.07.18.47`.
- Контекст версии: после исправления swap и исчезновения остальные короткие эффекты всё ещё могли завершиться по wall-clock до первого мобильного paint, а reward-окно могло сразу оказаться в финальном CSS-состоянии. Не стоит возвращать падение, тряску или общую reward-последовательность к независимым CSS-анимациям и таймерам без проверки реально показанных промежуточного и финального кадров.
- Затронутые файлы: battle animation/resolution, map rewards, общие стили, browser-smoke, документация боя и данных, `VERSIONS.md`, `version.json`.
- Проверка: синтаксис JS; battle/project/encoding-проверки; отдельные browser-smoke для боя, обычной награды и level-up с покадровыми assertions.
