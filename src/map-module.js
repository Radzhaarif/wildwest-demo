...
function generateMap(config) {
  // Генерация карты намеренно полностью строится из mapConfig: уровни, веса
  // событий, payload событий, дороги и босс. Это позволяет менять карту через
  // data/maps/*.jsonc без правки JS.
...
function getMapLevelSummary(config) {
  const levelConfigs = getMapLevelConfigs(config);
...
function createNode(config, levelNumber, index, eventConfig) {
  // Сначала настройка уровня выбирает событие точки, потом payload этого типа по диапазону уровней.
...
function pickEventPayload(config, eventName, eventType, levelNumber) {
  // Разделы событий имеют одинаковую идею: набор вариантов
  // с minLevel/maxLevel/weight. Если подходящих вариантов нет, событие получит
  // пустой payload и обработчик должен пережить это спокойно.
...
function connectLevels(currentNodes, nextNodes, pathConfig) {
  // Инварианты графа карты:
  // 1. каждая точка текущего уровня имеет хотя бы один путь вперед;
  // 2. каждая точка следующего уровня имеет хотя бы один входящий путь;
  // 3. дополнительные связи ограничены maxConnectionsFromNode;
  // 4. ближайшие по горизонтали цели выбираются первыми, чтобы дороги меньше
  // пересекались и карта оставалась читаемой.
...
function pickMapEvent(events) {
  // Нулевые веса полностью выключают тип события. Это удобно для тестов, когда
  // в карте временно оставляют только skip/shop/heal.
...
function pickWeightedArray(items) {
  // Возвращаем structuredClone, чтобы runtime-изменения payload события не
  // мутировали исходный объект из mapConfig.
...
function getOrderedTargets(sourceIndex, sourceCount, targets) {
  // Порядок целей строится по относительной позиции источника. Это не строгий
  // алгоритм непересечения, но он стабилизирует дороги около ближайших узлов.
...
function getNearestSourceForTarget(target, targets, sources) {
  return getOrderedSourcesForTarget(target, targets, sources)[0];
}
